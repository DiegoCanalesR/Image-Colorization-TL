...
from keras.layers import Input, Conv2D, UpSampling2D, concatenate, Reshape
from keras.preprocessing.image import img_to_array, load_img
from skimage.color import rgb2lab, lab2rgb, rgb2gray
from preprocesamiento import path_imags
from herramientas import resnet_embedding
import numpy as np
...
def train_model():
    ...
    y = Conv2D(2, (3,3), padding='same', activation='tanh')(y)
    y = UpSampling2D((2, 2))(y)
    
    model = Model(inputs=[inputs, resnet_embedding_input], outputs=[y])
    model.compile(optimizer='adam', loss='mse')         
    tensorboard = TensorBoard(log_dir="logs/modelo_2" + datetime.now().strftime("%Y%m%d-%H%M%S"))
...
def test_model():
    ...
    embedding = resnet_embedding(X)
    output = model.predict([X, embedding])
    output = output*128
